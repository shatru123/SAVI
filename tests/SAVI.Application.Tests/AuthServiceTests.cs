...
public class AuthServiceTests
{
    ...
    {
        Environment.SetEnvironmentVariable("SAVI_OWNER_KEY", "TestSecretAdminKey!999");
        try
        {
            var request = new RegisterRequestDto
            {
                ...
                AdminKey = "TestSecretAdminKey!999"
            };
        ...
        {
            Environment.SetEnvironmentVariable("SAVI_OWNER_KEY", null);
        }
    }
    ...
    {
        Environment.SetEnvironmentVariable("SAVI_OWNER_KEY", "ValidConfiguredSecretKey");
        try
        {
            var request = new RegisterRequestDto
            {
    ...
    [Fact]
    public async Task RegisterAsync_WhenAdminKeyNotConfigured_ReturnsError()
    {
        Environment.SetEnvironmentVariable("SAVI_OWNER_KEY", null);
        Environment.SetEnvironmentVariable("SAVI_ADMIN_KEY", null);

        var request = new RegisterRequestDto
        {
            DisplayName = "Operator",
            Email = "operator@savi.net",
            Password = "Password123!",
            ConfirmPassword = "Password123!",
            AdminKey = "AnyKeyProvided"
        };

        _mockUserRepo.Setup(r => r.GetByEmailAsync("operator@savi.net", It.IsAny<CancellationToken>()))
            .ReturnsAsync((User?)null);

        var result = await _authService.RegisterAsync(request);

        Assert.False(result.Success);
        Assert.Contains("not configured on this server", result.Error, StringComparison.OrdinalIgnoreCase);
        _mockUserRepo.Verify(r => r.AddAsync(It.IsAny<User>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}
